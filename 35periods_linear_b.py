"""
APM Project 2026 - Assignment 6b: Realized demand evaluation (35 periods, linear extrapolation)

Uses the production plan from 6a (linear, optimized on forecast) and evaluates
it against REALIZED demand. Weeks 31-35 generated via linear least-squares
regression. Costs rescaled to 30-week equivalent using D = 30/35.
...
# ── Capacity / cost parameters (same as 5a) ───────────────────────────
CAP_X_BASE     = 800
CAP_X_MAX_EXP  = 200
COST_EXP_X     = 10
CAP_X_OT_MAX   = 300
...
backorder_30  = math.ceil(total_backorder_35 * D)   # round UP, conservative
...
# Use wider columns for the comparison table
...
for label, key, fmt, is_total in cmp_rows:
    ...
    if v5 is not None:
        ...
        # For service level / fill rate: lower is worse, so colour logic flips
        is_kpi = key in ("sl", "fr")
