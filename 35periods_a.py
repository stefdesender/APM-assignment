"""
APM Project 2026 - Assignment 6a (extended horizon, 35 periods)

Identical model as Assignment 5a, but with the planning horizon extended
from 30 to 35 weeks. The 5 extra weeks of demand were generated by linear
extrapolation (see 35periods_generator.py). Costs are rescaled to a
30-week equivalent using factor D = 30/35 to allow a fair comparison
with the original 5a result.

NOTE: investment costs (permanent capacity expansion on X and Y) are
one-time investments and are NOT rescaled - they are independent of the
horizon length.

Since this script uses the FORECASTED demand (which is fully met by the
plan), service level and fill rate are 100% by construction. They become
meaningful in 6b, where the plan is evaluated against realized demand.
"""
