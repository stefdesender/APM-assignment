"""
APM Project 2026 - Assignment 6b (extended horizon, 35 periods)

Identical setup as 5b, but with the planning horizon extended from 30 to
35 weeks. The 5 extra weeks of demand were generated by linear
extrapolation (see 35periods_generator.py).

Workflow:
  1. Re-solve 6a internally to obtain the optimal 35-week plan.
  2. Simulate the plan against the (extrapolated) realized demand.
  3. Compute service level / fill rate over the FIRST 30 WEEKS only,
     so the metrics remain comparable to 5b. The 5 extra weeks are
     fictional buffer periods, not part of the evaluation.
  4. Rescale time-dependent costs to a 30-week equivalent via factor
     D = 30/35. Investment costs are NOT rescaled (one-time).
     Backorder unit counts are rounded UP via math.ceil to remain
     conservative.
  5. Read Output_5b from OUTPUT.xlsx and produce a side-by-side
     comparison table.
"""
