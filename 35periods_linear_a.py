"""
APM Project 2026 - Assignment 6a: Extended horizon (35 periods, linear extrapolation)

Planning horizon extended from 30 to 35 weeks. Weeks 31-35 generated via
linear least-squares regression on the 30-week demand series. Optimized on
FORECASTED demand. Costs rescaled to 30-week equivalent using D = 30/35.
Investment costs are NOT rescaled (one-time, horizon-independent).
"""
...
if model.status == GRB.OPTIMAL:
    ...
    setup_30    = total_setup    * D
    holding_30  = total_holding  * D
    ot_x_30     = total_ot_x     * D
    ot_y_30     = total_ot_y     * D
    total_30    = setup_30 + holding_30 + ot_x_30 + ot_y_30 + invest_X + invest_Y

    # ── Service metrics (trivially 100% on forecast) ───────────────────
    service_level = 1.0   # all forecast periods fully served
    fill_rate     = 1.0   # all forecast units delivered

    OUTPUT_FILE = "OUTPUT.xlsx"
    SHEET_NAME  = "Output_linear_forecast"

    # ── Try to read existing 5a results for comparison ─────────────────
    sol_5a = None
    if os.path.exists(OUTPUT_FILE):
        wb_check = load_workbook(OUTPUT_FILE, data_only=True)
        if "Output_5a" in wb_check.sheetnames:
            ws5a = wb_check["Output_5a"]
            # Cost summary lives in rows 3..10, column C in 5a
            try:
                sol_5a = {
                    "setup":    float(ws5a.cell(3, 3).value or 0),
                    "holding":  float(ws5a.cell(4, 3).value or 0),
                    "invest_X": float(ws5a.cell(5, 3).value or 0),
                    "invest_Y": float(ws5a.cell(6, 3).value or 0),
                    "ot_x":     float(ws5a.cell(7, 3).value or 0),
                    "ot_y":     float(ws5a.cell(8, 3).value or 0),
                    "total":    float(ws5a.cell(10, 3).value or 0),
                }
            except (TypeError, ValueError):
                sol_5a = None

    # ── Open / create the workbook for writing ─────────────────────────
    if os.path.exists(OUTPUT_FILE):
    ...
    plain(ws.cell(1, 2),
          "Assignment 6a - Extended horizon (35 periods, linear extrapolation) - Forecast evaluation",
          bold=True, size=13)

    # ── Cost summary (35-week raw) ─────────────────────────────────────────
    ws.row_dimensions[2].height = 4
    cost_rows = [
        ("Setup cost (35w)",        total_setup,                 False),
        ("Holding cost (35w)",      total_holding,               False),
        ("Investment cost X",       invest_X,                    False),
        ("Investment cost Y",       invest_Y,                    False),
        ("Overtime cost X (35w)",   total_ot_x,                  False),
        ("Overtime cost Y (35w)",   total_ot_y,                  False),
        ("Total overtime (35w)",    total_ot_x + total_ot_y,     False),
        ("Total cost (35w)",        model.ObjVal,                True),
    ]
    ...
    rescaled_rows = [
        ...
        ("Total cost (30w eq.)",      total_30,    True),
    ]
    start_row = 3 + len(cost_rows) + 1
    plain(ws.cell(start_row, 2), "Rescaled to 30-week equivalent  (factor D = 30/35)",
          size=8, color="888888")
    start_row += 1
    ...
    plain(ws.cell(metrics_start, 2), "Service metrics (on forecasted demand)",
          size=8, color="888888")
    metrics_start += 1
    metric_rows = [
        ("Service level", service_level, '0.00%'),
        ("Fill rate",     fill_rate,     '0.00%'),
    ]
    for r, (label, val, fmt) in enumerate(metric_rows, start=metrics_start):
        ws.row_dimensions[r].height = 16
        plain(ws.cell(r, 2), label, size=9, color="555555")
        plain(ws.cell(r, 3), val, size=9, fmt=fmt)
        ws.merge_cells(f"C{r}:{get_column_letter(last_col)}{r}")

    # ── Comparison table 5a vs 6a ─────────────────────────────────────────
    comp_start = metrics_start + len(metric_rows) + 2
    if sol_5a is not None:
        section_title(ws, comp_start, last_col, "Comparison: 5a (30w) vs 6a (30w equivalent)")
        comp_start += 1
        ws.row_dimensions[comp_start].height = 16
        hdr(ws.cell(comp_start, 2), "Cost type")
        hdr(ws.cell(comp_start, 3), "5a")
        hdr(ws.cell(comp_start, 4), "6a (30w eq.)")
        hdr(ws.cell(comp_start, 5), "Diff")
        hdr(ws.cell(comp_start, 6), "Diff %")

        comp_rows = [
            ("Setup",       sol_5a["setup"],    setup_30),
            ("Holding",     sol_5a["holding"],  holding_30),
            ("Investment X", sol_5a["invest_X"], invest_X),
            ("Investment Y", sol_5a["invest_Y"], invest_Y),
            ("Overtime X",  sol_5a["ot_x"],     ot_x_30),
            ("Overtime Y",  sol_5a["ot_y"],     ot_y_30),
            ("Total",       sol_5a["total"],    total_30),
        ]
        for r, (label, v5, v6) in enumerate(comp_rows, start=comp_start + 1):
            ws.row_dimensions[r].height = 15
            is_total = (label == "Total")
            diff = v6 - v5
            pct = (diff / v5) if v5 else 0
            plain(ws.cell(r, 2), label, size=9, bold=is_total, border=bot_thin)
            plain(ws.cell(r, 3), round(v5, 2),   size=9, bold=is_total, align="right", fmt='"€"#,##0.00', border=bot_thin)
            plain(ws.cell(r, 4), round(v6, 2),   size=9, bold=is_total, align="right", fmt='"€"#,##0.00', border=bot_thin)
            color = "CC0000" if diff > 0 else ("117733" if diff < 0 else "000000")
            plain(ws.cell(r, 5), round(diff, 2), size=9, bold=is_total, align="right",
                  color=color, fmt='"€"#,##0.00;[Red]-"€"#,##0.00', border=bot_thin)
            plain(ws.cell(r, 6), pct, size=9, bold=is_total, align="right",
                  color=color, fmt='+0.0%;-0.0%;0.0%', border=bot_thin)

        param_start = comp_start + len(comp_rows) + 3
    else:
        plain(ws.cell(comp_start, 2),
              "Comparison with 5a not available (run assignment_5a.py first to populate Output_5a sheet).",
              size=9, color="AA0000")
        param_start = comp_start + 2

    # ── Expansion & overtime parameters ───────────────────────────────────
    ws.row_dimensions[param_start].height = 6
    param_rows = [
        ("X  base capacity",       f"{CAP_X_BASE} units / week"),
        ("X  permanent expansion", f"+{round(dx_val)} units  @ EUR {COST_EXP_X} / unit  (max {CAP_X_MAX_EXP})"),
        ("X  new base capacity",   f"{round(cap_x_new)} units / week"),
        ("X  overtime max",        f"{CAP_X_OT_MAX} units / week  @ EUR {COST_OT_X} / unit"),
        ("Y  base capacity",       f"{CAP_Y_BASE} min / week"),
        ("Y  permanent expansion", f"+{round(dy_pct_val, 1)}%  @ EUR {COST_EXP_Y_PCT:,} / %  (max {CAP_Y_MAX_PCT}%)"),
        ("Y  new base capacity",   f"{round(cap_y_new)} min / week"),
        ("Y  overtime max",        f"{CAP_Y_OT_MAX} h / week  @ EUR {COST_OT_Y} / hour"),
    ]
    for r, (label, val) in enumerate(param_rows, start=param_start + 1):
        ws.row_dimensions[r].height = 16
        plain(ws.cell(r, 2), label, size=9, color="555555")
        plain(ws.cell(r, 3), val, size=9, bold=("new base" in label))
        ws.merge_cells(f"C{r}:{get_column_letter(last_col)}{r}")

    r = param_start + len(param_rows) + 2
    ...
    print()
    print(f"=== 30-week equivalent (factor D = 30/35, investments NOT scaled) ===")
    print(f"Total cost:      EUR {total_30:,.2f}")
    ...
    if sol_5a is not None:
        print()
        print(f"=== Comparison 5a vs 6a (30-week equivalent) ===")
        print(f"{'Cost type':<14}{'5a':>14}{'6a':>14}{'Diff':>14}{'Diff %':>10}")
        print("-" * 66)
        comp = [
            ("Setup",       sol_5a["setup"],    setup_30),
            ("Holding",     sol_5a["holding"],  holding_30),
            ("Invest X",    sol_5a["invest_X"], invest_X),
            ("Invest Y",    sol_5a["invest_Y"], invest_Y),
            ("Overtime X",  sol_5a["ot_x"],     ot_x_30),
            ("Overtime Y",  sol_5a["ot_y"],     ot_y_30),
            ("Total",       sol_5a["total"],    total_30),
        ]
        for label, v5, v6 in comp:
            diff = v6 - v5
            pct = (diff / v5 * 100) if v5 else 0
            print(f"{label:<14}{v5:>14,.2f}{v6:>14,.2f}{diff:>+14,.2f}{pct:>+9.1f}%")
    else:
        print()
        print("(Run assignment_5a.py first to enable side-by-side comparison.)")