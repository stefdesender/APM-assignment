...
# ── Scenarios to run ─────────────────────────────────────────────────────
SCENARIOS = [
    {"label": "95% CSL", "z": 1.65, "sheet": "ss_realized_95"},
    {"label": "99% CSL", "z": 2.33, "sheet": "ss_realized_99"},
]
...
for scenario in SCENARIOS:
    ...
    plain(ws.cell(1, 2),
          f"Assignment 6a - Realized demand (safety stock, {label})",
          bold=True, size=13)
    ...
    # Service metrics
    row += 1
    section_title(ws, row, last_col, f"Service metrics (end product, realized demand, {label})")
    row += 1
    ...
    # Demand vs delivered
    row += 2
    section_title(ws, row, last_col, "Demand vs delivered (end product)")
    row += 1
    hdr(ws.cell(row, 2), "")
    for t in periods: hdr(ws.cell(row, t+2), str(t))
    row += 1
    plain(ws.cell(row, 2), "Demand", size=9, border=bot_thin)
    for t in periods:
        plain(ws.cell(row, t+2), DEMAND_REALIZED[t-1], size=9,
              align="center", fmt='#,##0', border=bot_thin)
    row += 1
    plain(ws.cell(row, 2), "Delivered", size=9, border=bot_thin)
    prev_bo = 0.0
    for t in periods:
        new_bo_t = max(0.0, backorders[t] - prev_bo)
        delivered = DEMAND_REALIZED[t-1] - new_bo_t
        shortage  = delivered < DEMAND_REALIZED[t-1]
        plain(ws.cell(row, t+2), round(delivered), size=9, align="center",
    ...
    # Production schedule
    row += 2
    section_title(ws, row, last_col, "Production / order schedule (units, from 6a plan)")
    row += 1
    hdr(ws.cell(row, 2), "Part")
    for t in periods: hdr(ws.cell(row, t+2), str(t))
    for i in PARTS:
    ...
    row += 2
    section_title(ws, row, last_col, "Inventory levels (end of period, realized)")
    row += 1
    hdr(ws.cell(row, 2), "Part")
    for t in periods: hdr(ws.cell(row, t+2), str(t))
    for i in PARTS:
...
if len(results) == 2:
    ...
    print(f"\n{'='*74}")
    print(f"  COMPARISON: {r95['label']} vs {r99['label']}")
    print(f"{'='*74}")
