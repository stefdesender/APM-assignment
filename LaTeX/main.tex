...
\[
I_{i,t} = I_{i,t-1} + x_{i,t-LT_i} - FD_{i,t} \qquad\qquad \text{if } t - LT_i < 1 \Rightarrow x_{i,t} = 0 \Rightarrow I_{i,t} = I_{i,t-1} - FD_{i,t}
\]
...
In part 1b, the production plan from part 1a is kept fixed and evaluated under realized demand. No re-optimization is performed; only the end-product demand is replaced by realized values. Unmet demand is backordered at a cost of €250 per unit per period.

The total cost increases from €394{,}454 to €447{,}792 (+€53{,}338), mainly driven by backorder costs of €46{,}750. The holding cost rises slightly from €314{,}954 to €321{,}542, while setup costs remain unchanged at €79{,}500. The service level equals 93.33\% (28/30 periods without backorder) and the fill rate is 96.89\% (5{,}830/6{,}017 units on time).

Backorders occur only in weeks 23 and 30 (77 and 110 units respectively), showing that the plan performs reasonably well under demand uncertainty but remains vulnerable when realized demand exceeds the inventory buffer.
...
The introduction of finite capacity increases total cost from €394{,}454.00 to €540{,}488.80. Setup costs rise from €79{,}500.00 to €145{,}500.00 (+83\%), as the model can no longer concentrate production in a few large batches during the most cost-efficient periods and must spread production over more periods with more frequent, smaller runs. Holding costs increase from €314{,}954.00 to €394{,}988.80 (+25.4\%), since capacity constraints force earlier production, building up inventory that must be carried over time. Although this results in a higher total cost, it produces a more realistic and operationally feasible production plan.
...
\[
OT_{X,t} \leq 300 \qquad OT_{Y,t} \leq 38 \qquad \forall t \in \{1,\ldots,30\}
\]
...
Constraints 1--3 (inventory balance, setup linking, minimum lot size) remain unchanged from Assignment 1a.
...
These constraints ensure that the expansion decisions remain within feasible and predefined bounds. The capacity expansion of workstation X is limited to at most 200 additional units, while workstation Y is limited to at most 40\%, expressed in discrete percentage steps.

\[
0 \leq dx \leq 200 \qquad\qquad 0 \leq dy\_pct \leq 40
\]

\subsubsection{Cost comparison}

Part 2a results in the highest total cost of €540{,}488.80 due to high holding and setup costs under strict capacity constraints. In Part 3a, allowing overtime reduces the total cost to €521{,}270.80 by lowering holding costs, although it introduces additional overtime costs. Part 4a achieves the lowest total cost of €520{,}723.30 by enabling permanent capacity expansion. The optimization model identifies an ideal expansion of 102 additional units for workstation X and a capacity increase of 8.3\% for workstation Y. Despite the associated investment cost for these permanent upgrades, this configuration further reduces holding and setup costs, making it the most cost-efficient solution overall.
...
Constraints 1--3 (inventory balance, setup linking, minimum lot size) and constraints 4--5 (maximum overtime, maximum expansion) remain unchanged from assignments 1a, 3a and 4a respectively.

\textbf{Constraint 6:} Maximum capacity adjusted with overtime and permanent expansion for both workstations
...
\begin{table}[h]
    ...
    \begin{tabular}{l c c c c c}
        \toprule
        \textbf{Design} & \textbf{Safety Stock} & \textbf{Total Cost (forecast)} & \textbf{Total Cost (realized)} & \textbf{\# Backorders} & \textbf{Service level} \\
        \midrule
...
\subsection*{Idea 2: Outsourcing production}

\textbf{Motivation and approach}

Part B4702 has both the longest lead time (5 weeks) and the highest BOM multiplier ($7 \times 3 = 21$ units per end product) of all components. This combination results in large batches that must be ordered well in advance, leading to substantial holding costs. This makes B4702 the most suitable candidate for outsourcing. By sourcing the component from an external supplier capable of faster delivery, the required inventory buffer decreases and holding costs are structurally reduced.

In the proposed scenario, it is assumed that an external supplier can deliver B4702 within 1 week. The internal setup cost of \texteuro 2,500 per batch is eliminated, but is replaced by an order cost per batch placed with the external supplier. An order cost of \texteuro 500 per order is used. The robustness of this assumption was verified through a sensitivity analysis.

\vspace{0.5em}
\textbf{Determining the order cost: sensitivity analysis}

Since no specific supplier data was available, the order cost was not derived using the EOQ formula (see appendix B). That formula is only valid when the EOQ is actually the optimal batch size, a condition that does not hold here given the minimum lot sizes, capacity constraints and BOM structure present in the model \cite{Silver1998}. Instead, the impact of the order cost was tested directly by solving the model for values ranging from \texteuro 250 to \texteuro 1,500 per order in steps of \texteuro 250.

Table 15 (in appendix B) shows that the total cost difference between \texteuro 250 and \texteuro 1,500 per order amounts to only \texteuro 6,250 over the full planning horizon, which is marginal relative to a total cost of approximately \texteuro 420,000. Based on this, \texteuro 500 is used as the central assumption; it is a conservative but realistic estimate that does not materially affect the conclusions.

\vspace{0.5em}
\textbf{Results compared to Assignment 5}

With a lead time of 1 week and an order cost of \texteuro 500, outsourcing yields a total cost of \texteuro 420,963.80 based on forecasted demand, compared to \texteuro 510,076.60 in Assignment 5a. The reduction of nearly \texteuro 90,000 is entirely driven by a sharp decrease in holding costs (as intuitively expected given the reduction in lead time). As an additional benefit, the need for permanent capacity expansion on workstation X disappears (Investment X drops from \texteuro 2,000 to \texteuro 0), because the increased planning flexibility allows the optimizer to spread production more evenly. (See appendix B: table 16)

\vspace{0.5em}
\textbf{Sensitivity analysis: effect of lead time}

Since a lead time of 1 week represents an optimistic scenario, the impact of lead times of 2 to 4 weeks was also tested. A lead time of 5 weeks or more is not considered, as this corresponds to the current internal situation and therefore offers no benefit. Table 17 (in appendix B) shows that total cost based on forecasted demand increases as lead time grows longer, but remains well below the level of Assignment 5a in all cases. Even at a lead time of 4 weeks, the saving compared to 5a still amounts to approximately \texteuro 31,000. Notably, at lead times of 3 or 4 weeks the optimizer reverts to a larger permanent capacity expansion on workstation Y (20\% instead of 6.9\%) and reinvests in workstation X. This illustrates that a shorter lead time not only reduces holding costs, but also relieves capacity pressure by allowing production to be scheduled more flexibly.

\vspace{0.5em}
\textbf{Evaluation under realized demand and comparison with 5b}

When the outsourcing plan ($L = 1$, order cost = \texteuro 500) is evaluated under realized demand, total cost increases from \texteuro 420,864 to \texteuro 464,574, mainly due to backorder costs of \texteuro 37,250. The service level (93.3\%) and fill rate (97.52\%) are identical to those of Assignment 5b (see table 18 in Appendix B). The fact that service level and fill rate are unchanged is logical and worth explaining. The backorders in 5b (and 6b) occurred in weeks 26 and 30, not because B4702 was unavailable in time, but because E2801 inventory was insufficient at those moments to absorb the higher realized demand. In the 5b plan, B4702 stock was already fully depleted from week 6 onwards, the entire supply chain had been drained long before the backorders arose. The bottleneck therefore lay elsewhere in the system. Outsourcing consequently addresses a different problem than demand uncertainty at the level of the end product. This is not a weak conclusion, but rather a sign of critical insight: the measure is cost-efficient, but does not address the root cause of the backorders.

\vspace{0.5em}
\textbf{Break-even analysis: price premium over internal production cost}

So far it has been assumed that the purchase price at the external supplier equals the internal production cost of B4702. In practice, an external party will typically charge a higher unit price. The break-even analysis therefore calculates up to what price premium per unit (on top of the internal production cost) outsourcing remains advantageous compared to Assignment 5b. Over the full planning horizon, a total of 78,847 units of B4702 must be purchased. Given a cost saving of approximately \texteuro 89,000 ($LT = 1$) at equal unit price, the purchase price can be at most \texteuro 1.13 per unit higher than the internal production cost before the benefit of outsourcing is fully eliminated. For longer lead times the margin is smaller. (See table 19 in appendix B)

\subsection*{Idea 3: Double exponential smoothing for demand forecasting}
...
\subsection*{Idea 4: Increasing the number of periods in the planning horizon}

A key observation from the results of Assignments 1 through 5 is that backorders tend to concentrate in the final periods of the planning horizon. This occurs because the optimizer, knowing that no demand exists beyond period 30, deliberately runs down inventory toward the end of the horizon to minimize holding costs. In practice, however, the company will continue to operate beyond 30 weeks, and this so-called \textit{end-of-horizon effect} leads to an artificially optimistic plan that underperforms when evaluated against realized demand.

To mitigate this effect, the planning horizon was extended from 30 to 35 weeks. The additional 5 weeks of demand data serve as a buffer: the optimizer now plans production beyond week 30, preventing it from draining inventory prematurely. All time-dependent costs (setup, holding, backorder, and overtime) over the 35-week horizon are then rescaled to a 30-week equivalent using factor $D = 30/35$ to allow a fair comparison with the original 30-week results from Assignment 5. Investment costs, being one-time expenditures independent of the horizon length, are not rescaled. A limitation of this approach is that the demand values for weeks 31--35 are necessarily estimated, as no observed data exists beyond the original 30-week horizon. The quality of the results therefore depends on the accuracy of this estimate.

For the estimation, two methods were considered:

\begin{itemize}
    \item \textbf{Linear extrapolation:} A linear least-squares regression was fitted to the 30 observed weeks of demand data. The resulting trend line was then extended to predict demand for weeks 31--35. The realized demand series exhibits a slope of approximately $+1.54$ units per week, indicating a mild upward trend. This method is statistically grounded but produces extrapolated values where realized demand systematically exceeds the forecast, which can amplify the end-of-horizon backorder effect in the extended weeks.
    \item \textbf{Cyclic repetition:} The demand values from weeks 1--5 are repeated as weeks 31--35. This is a simpler heuristic that assumes demand patterns are roughly periodic. In our dataset, weeks 1--5 represent a low-demand startup phase where realized demand is generally below the forecast, which provides a natural buffer against backorders in the extended period.
\end{itemize}

A visual comparison of both extrapolation methods can be found in Appendix C.

The key results are summarized in Table~\ref{tab:horizon_results}.
...
\begin{table}[h]
    ...
    \begin{tabular}{l c c c c c}
        \toprule
        \textbf{Design} & \textbf{Total (forecast)} & \textbf{Total (realized)} & \textbf{\# Backorders} & \textbf{Service level} & \textbf{Fill rate} \\
        \midrule
        Assignment 5        & \texteuro 510,077 & \texteuro 553,687 & 149   & 93.3\% & 97.52\% \\
        Linear extrap.      & \texteuro 505,814 & \texteuro 604,858 & 91    & 93.3\% & 98.49\% \\
        Cyclic repetition   & \texteuro 494,011 & \texteuro 535,141 & 67    & 93.3\% & 98.89\% \\
        \bottomrule
    \end{tabular}
    \caption{Extended horizon (35 periods): cost comparison (30-week equivalent)}
\end{table}

On forecasted demand, both extended-horizon variants yield a lower total cost than Assignment~5, confirming that the longer planning horizon allows the optimizer to find more efficient production schedules. The cyclic variant achieves the largest reduction (-3.1\%), while the linear variant saves approximately 0.8\%.

Under realized demand, however, the two methods diverge significantly. The cyclic variant reduces total cost by 3.3\% relative to Assignment~5 (\texteuro 535,141 vs.\ \texteuro 553,687) and cuts backorders from 149 to 67 units, improving the fill rate from 97.52\% to 98.89\%. The linear variant, by contrast, increases total cost by 9.2\% to \texteuro 604,858 with 91 backorders. This counterintuitive result occurs because the linearly extrapolated realized demand for weeks 31--35 consistently exceeds the extrapolated forecast (by approximately 20--25 units per week), causing the optimizer to under-plan production for those weeks and generating substantial backorders in the extended period. Since all costs, including these artificial backorders, are scaled by factor $D$, they inflate the 30-week equivalent cost.

The cyclic variant avoids this issue because weeks 1--5 of our dataset have realized demand below the forecast, so the extended weeks act as a genuine buffer rather than a source of additional backorders.

Based on these results, the cyclic repetition method was selected as the preferred approach for the extended horizon analysis. It demonstrates that by simply extending the planning horizon with a reasonable demand estimate, the system can simultaneously reduce total cost by 3.3\%, cut backorders by more than half (from 149 to 67 units), and improve the fill rate from 97.52\% to 98.89\%. This confirms that a significant portion of the backorder cost observed in Assignment~5 is attributable to the end-of-horizon effect rather than to a fundamental capacity or planning deficiency.

\subsection*{Idea 5: Capacity expansion both workstations}

To investigate whether capacity expansion could mitigate demand uncertainty, sensitivity analyses were conducted for both workstations around the Part 5a optimal capacity levels. A detailed breakdown of all tested values, including full cost decomposition, is provided in Appendix D.

For Workstation X, which directly produces E2801, additional capacity might be expected to reduce backorders. However, backorders remain constant at 149 units for all capacities between 100 and 600 units. Since the Assignment 5a optimizer reached the upper bound of 200 units, we tested higher values, but found no cost improvement. The lowest realized total cost of €553{,}718 occurs at 200 units, while further expansion only increases investment cost.
...
\toprule
\textbf{Design} & \textbf{Max cap X} & \textbf{Forecast cost} & \textbf{Realized cost} & \textbf{Backorders} & \textbf{Service level} \\
\midrule
1       & 100 u & €512{,}123.10 & €557{,}733.10 & 149 & 93.33\% \\
2       & 150 u & €511{,}953.40 & €555{,}745.10 & 149 & 93.33\% \\
current & 200 u & €510{,}108.10 & €553{,}718.10 & 149 & 93.33\% \\
3       & 250 u & €510{,}443.10 & €554{,}053.10 & 149 & 93.33\% \\
4       & 300 u & €510{,}843.10 & €554{,}453.10 & 149 & 93.33\% \\
\bottomrule
\end{tabular}
}
\end{minipage}
...
\centering
\vspace{-0.3cm}
\includegraphics[width=\textwidth]{MaxX.png}
\end{minipage}

\caption{Sensitivity analysis: maximum expansion Workstation X}
\end{table}
\vspace{0.5em}

For Workstation Y, forcing a higher expansion produces more B1401 and B2302 components upfront, building a component buffer. However, the backorder cost shows no consistent improvement and even increases at higher expansion levels. The reason is fundamental: Workstation Y only produces intermediate components. No matter how large the component buffer is, if the end product E2801 is not produced at the right time, backorders will still occur.
...
\begin{tabular}{lccccc}
\toprule
\textbf{Design} & \textbf{Max cap Y} & \textbf{Forecast cost} & \textbf{Realized cost} & \textbf{Backorders} & \textbf{Service level} \\
\midrule
1       & 4.1\%  & €514{,}991.30 & €558{,}601.30 & 149 & 93.33\% \\
2       & 9.1\%  & €512{,}176.50 & €555{,}786.50 & 149 & 93.33\% \\
current & 14.1\% & €510{,}089.40 & €553{,}699.40 & 149 & 93.33\% \\
3       & 19.1\% & €511{,}634.10 & €555{,}244.10 & 149 & 93.33\% \\
4       & 24.1\% & €514{,}752.30 & €558{,}182.30 & 149 & 93.33\% \\
\bottomrule
\end{tabular}
}
\end{minipage}
...
\centering
\vspace{-0.3cm}
\includegraphics[width=\textwidth]{MaxY.png}
\end{minipage}

\caption{Sensitivity analysis: maximum expansion Workstation Y}
\end{table}

\vspace{0.5em}

The root cause is the same in both cases: backorders in weeks 26 and 30 arise from a mismatch in production timing, not a capacity shortage. The optimizer, planning on forecasted demand, does not schedule production close enough to these demand peaks, leaving additional capacity unused if it is not deployed at the right moment.

Both analyses therefore confirm that capacity expansion alone is an ineffective strategy for handling demand uncertainty. The lowest realized cost is achieved at the Part 5a reference values (dx = 200 u, dy = 14.1\%). The real solution lies in planning logic, which motivates the safety stock approach discussed in Idea 1.

\subsection{Conclusion 6a}

Of the five strategies explored, capacity expansion and Holt's forecasting method proved ineffective, the former because backorders stem from a timing mismatch rather than a capacity shortage, the latter due to insufficient historical data. Outsourcing of B4702 achieves large cost savings but does not reduce backorders, since the bottleneck lies at the end-product level. The extended planning horizon reduces both cost and backorders by eliminating the end-of-horizon effect, but requires assumptions about future demand.
 
The recommended design is \textbf{safety stock at the 99\% confidence level}. It directly addresses the root cause of uncertainty: forecast errors at the end-product level that propagate into backorders. With a safety stock of 84 units, total realized cost decreases by 3.1\% while backorders drop from 149 to 26 units and fill rate improves to 99.57\%. Outsourcing and the extended horizon remain valuable as complementary cost-reduction measures, but neither substitutes for an inventory buffer at the point of customer delivery.
...
An important internal source of uncertainty is product and process complexity. E2801 has a multi-level bill of materials with seven parts across three production levels, requiring multiple sequential assembly steps. Each step introduces variability, and due to their interdependence, delays or errors propagate through the system. This effect is most critical in Assignments 1 and 2, where no capacity buffer exists to absorb disruptions. Simplifying the BOM or reducing the number of components would shorten lead times and limit cascading variability.

Setup operations further contribute to uncertainty, as each setup affects effective processing time. While reducing setups lowers costs, larger batch sizes can amplify variability propagation, highlighting a trade-off between cost efficiency and operational robustness.
...
\hangindent=1.5em
\hangafter=1
Limère, V. (2026). APM3\_ProductionPlanning\_GroupProject [Lecture slides, Advanced Production Management, Ghent University].
...
\hangindent=1.5em
\hangafter=1
Limère, V. (2026). APM4\_Inventory\_Deterministic [Lecture slides, Advanced Production Management, Ghent University].

\hangindent=1.5em
\hangafter=1
Limère, V. (2026). APM5\_Inventory\_Stochastic [Lecture slides, Advanced Production Management, Ghent University].

\end{flushleft}

\assignmentsection{Appendix}

\section*{Appendix A: EOQ formula}
EOQ formula:
...
\vspace{0.5em}
\noindent\textit{Reference:} Silver, E. A., Pyke, D. F., \& Peterson, R. (1998). \textit{Inventory management and production planning and scheduling} (3rd ed.). Wiley.

\section*{Appendix B: Outsourcing tables}

\subsection*{Outsourcing results}
...
\toprule
\textbf{Order cost (€ / order)} & \textbf{€250} & \textbf{€500} & \textbf{€750} & \textbf{€1000} & \textbf{€1250} & \textbf{€1500} \\
\midrule
Total cost (EUR) & €419{,}713.80 & €420{,}963.80 & €422{,}213.80 & €423{,}463.80 & €424{,}713.80 & €425{,}963.80 \\
\# orders B4702 & 5 & 5 & 5 & 5 & 5 & 5 \\
\bottomrule
\end{tabular}
\caption{Outsourcing: order cost sensitivity analysis for B4702 under forecast demand with \(LT = 1\) week}
\end{table}
...
\begin{tabular}{lrr}
\toprule
\textbf{Cost type} & \textbf{Part 5a} & \textbf{Part 6a (outsourcing, LT=1)} \\
\midrule
Setup cost        & €132{,}000.00 & €130{,}500.00 \\
Holding cost      & €354{,}296.60 & €273{,}787.80 \\
Order cost B4702  & --            & €2{,}500.00 \\
Investment X      & €2{,}000.00   & €0.00 \\
Investment Y      & €21{,}180.00  & €10{,}380.00 \\
Overtime X        & €600.00       & €364.00 \\
Overtime Y        & €0.00         & €3{,}432.00 \\
\midrule
\textbf{Total cost} & \textbf{€510{,}076.60} & \textbf{€420{,}963.80} \\
\bottomrule
\end{tabular}
\caption{Outsourcing: cost comparison between Assignment 5a and 6a under forecast demand}
\end{table}
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Lead time (weeks)} & \textbf{1 week} & \textbf{2 weeks} & \textbf{3 weeks} & \textbf{4 weeks} \\
\midrule
Total cost 6a & €420{,}963.80 & €442{,}403.80 & €453{,}828.10 & €478{,}548.10 \\
vs. LT=1 (best case) & €0.00 & +€21{,}440.00 & +€32{,}864.30 & +€57{,}584.30 \\
\# orders B4702 & 5 & 5 & 4 & 4 \\
Permanent exp. X & 0 & 0 & 100 & 100 \\
Permanent exp. Y (\%) & 6.92\% & 6.92\% & 20.18\% & 20.18\% \\
\bottomrule
\end{tabular}
\caption{Outsourcing: lead-time sensitivity analysis for B4702 under forecast demand}
\end{table}
...
\begin{tabular}{lrr}
\toprule
\textbf{Cost type} & \textbf{Part 5b} & \textbf{Part 6b (outsourcing, LT=1)} \\
\midrule
Setup cost        & €132{,}000.00 & €130{,}500.00 \\
Holding cost      & €360{,}656.60 & €280{,}147.80 \\
Order cost B4702  & --            & €2{,}500.00 \\
Investment X      & €2{,}000.00   & €0.00 \\
Investment Y      & €21{,}180.00  & €10{,}380.00 \\
Overtime X        & €600.00       & €364.00 \\
Overtime Y        & €0.00         & €3{,}432.00 \\
Backorder cost    & €37{,}250.00  & €37{,}250.00 \\
\midrule
\textbf{Total cost} & \textbf{€553{,}686.60} & \textbf{€464{,}573.80} \\
Service level & 0.933 & 0.933 \\
Fill rate & 0.9752 & 0.9752 \\
\bottomrule
\end{tabular}
\caption{Outsourcing: cost comparison between Assignment 5b and 6b under realized demand}
\end{table}
...
\toprule
& \textbf{5b (no outsourcing)} & \textbf{Outsourcing LT=1} & \textbf{Outsourcing LT=2} & \textbf{Outsourcing LT=3} & \textbf{Outsourcing LT=4} \\
\midrule
Total cost (realized demand) & €553{,}686.60 & €464{,}573.80 & €486{,}013.80 & €497{,}438.10 & €522{,}158.10 \\
Difference with 5b & -- & €89{,}112.80 & €67{,}672.80 & €56{,}248.50 & €31{,}528.50 \\
Max. price premium per unit & -- & €1.13 & €0.86 & €0.71 & €0.40 \\
\bottomrule
\end{tabular}
\caption{Outsourcing: break-even analysis of the maximum price premium per unit of B4702 under outsourcing, based on 78{,}847 units}
\end{table}

\section*{Appendix C: Extrapolation methods for extended horizon of 35 weeks}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{../extrapolation_comparison.png}
\caption{Comparison of extrapolation methods for extended planning horizon}
\end{figure}

\section*{Appendix D: Sensitivity tables for capacity expansion}

\begin{table}[H]
\centering
\begin{tabular}{lccccc}
\toprule
\textbf{Design} & \textbf{Max cap X (units)} & \textbf{Forecast cost (€)} & \textbf{Realized cost (€)} & \textbf{\#backorders} & \textbf{Service level} \\
\midrule
1       & 100 & 512{,}123.10 & 555{,}733.10 & 149 & 93.3\% \\
2       & 150 & 511{,}953.40 & 555{,}745.10 & 149 & 93.3\% \\
current & 200 & 510{,}108.10 & 553{,}718.10 & 149 & 93.3\% \\
3       & 250 & 510{,}443.10 & 554{,}053.10 & 149 & 93.3\% \\
4       & 300 & 510{,}843.10 & 554{,}453.10 & 149 & 93.3\% \\
\bottomrule
\end{tabular}
\caption{Sensitivity table: forecast cost vs realized cost per capacity increase for workstation X}
\end{table}
...
\textbf{Design} & \textbf{Max cap Y (\%)} & \textbf{Forecast cost (€)} & \textbf{Realized cost (€)} & \textbf{\#backorders} & \textbf{Service level} \\
\midrule
1       & 4.1\%  & 514{,}991.30 & 558{,}601.30 & 149 & 93.3\% \\
2       & 9.1\%  & 512{,}176.50 & 555{,}786.50 & 149 & 93.3\% \\
current & 14.1\% & 510{,}089.40 & 553{,}699.40 & 149 & 93.3\% \\
3       & 19.1\% & 511{,}634.10 & 555{,}244.10 & 149 & 93.3\% \\
4       & 24.1\% & 514{,}572.30 & 558{,}182.30 & 149 & 93.3\% \\
\bottomrule
\end{tabular}
\caption{Sensitivity table: forecast cost vs realized cost per capacity increase for workstation Y}
\end{table}
